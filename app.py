...
def h3(svg, text, size=20, bg="#e8f8e8", color="#2e7d32"):
...
ICO_FORM   = '<path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/><line x1="16" y1="13" x2="8" y2="13"/><line x1="16" y1="17" x2="8" y2="17"/>'
...
<style>
    ...
    .metric-card { background: linear-gradient(135deg, #0a2e0a, #0d3b0d); border: 1px solid #2e7d32;
    ...
    .section-header { display:flex; align-items:center; background: linear-gradient(90deg, #1b5e20, #2e7d32);
        padding: 10px 18px; border-radius: 8px; color: white; font-weight: 700; font-size: 1.1rem; margin-bottom: 12px; }
    .result-box { background: linear-gradient(135deg, #0a2e0a, #1a5c1a); border: 2px solid #81c784;
...
# ══════════════════════════════════════════════════════════
if page == "Predict":
    st.markdown(f"""<div class="page-title">{icon(ICO_TREE, 30, '#e8f8e8', '#2e7d32')}<h1>Forest Cover Type Prediction</h1></div>""", unsafe_allow_html=True)
    st.markdown("Enter the cartographic variables of a **30m × 30m** forest patch to predict its cover type using a **Random Forest** model.")
    ...
    with st.form("prediction_form"):
        ...
        with col1:
            elevation = st.number_input("Elevation (m)", min_value=1800, max_value=4000, value=2500)
    ...
    if submit_button:
        if model is None:
            ...
            if hasattr(model, 'predict_proba'):
                st.markdown("---")
                st.markdown(f'<h4 style="display:flex;align-items:center;">{icon(ICO_CHART, 18, "#e8f8e8", "#2e7d32")} Prediction Probabilities (All 7 Classes)</h4>', unsafe_allow_html=True)
                ...
                fig = go.Figure(go.Bar(x=class_labels, y=[p*100 for p in proba], marker_color=colors,
...
# ══════════════════════════════════════════════════════════
    ...
    with col1:
        ...
        st.markdown("""Predict the **type of forest cover** using cartographic analysis data for a **30m × 30m patch of land**
        in the Roosevelt National Forest, Colorado. Forest cover classification assists ecologists in planning sustainable land use.""")
...
# ══════════════════════════════════════════════════════════
    ...
    with c1: st.markdown('<div class="metric-card"><h2>~95%</h2><p>Accuracy</p></div>', unsafe_allow_html=True)
    with c2: st.markdown('<div class="metric-card"><h2>~94%</h2><p>Precision</p></div>', unsafe_allow_html=True)
    with c3: st.markdown('<div class="metric-card"><h2>~95%</h2><p>Recall</p></div>', unsafe_allow_html=True)
    with c4: st.markdown('<div class="metric-card"><h2>54</h2><p>Features</p></div>', unsafe_allow_html=True)

    st.markdown("<br>", unsafe_allow_html=True)
    col1, col2 = st.columns(2)
    with col1:
        st.markdown(h3(ICO_CPU, "Model: Random Forest"), unsafe_allow_html=True)
        st.markdown("""| Property | Details |\n|---|---|\n| Algorithm | Random Forest Classifier |\n| Ensemble Size | 100+ Decision Trees |\n| Features Used | 54 |\n| Classes | 7 cover types |\n| Serialization | Joblib (.pkl) |""")
        st.markdown(h3(ICO_LAYERS, "Why Random Forest?"), unsafe_allow_html=True)
        st.markdown("- Handles **mixed feature types** (continuous + binary) naturally\n- **Robust to overfitting** via bagging\n- Provides **feature importance** scores\n- Works well on **large, high-dimensional** datasets")
    with col2:
        st.markdown(h3(ICO_CHART, "Key Feature Importance"), unsafe_allow_html=True)
        ...
        fig = go.Figure(go.Bar(x=[v*100 for v in importance], y=features, orientation='h',
            marker_color='#66bb6a', text=[f"{v*100:.1f}%" for v in importance], textposition='outside'))
